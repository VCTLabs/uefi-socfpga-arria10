...
class VpdInfoFile:
    ...
    def Add(self, Vpd, Offset):
        ...
        if Vpd not in list(self._VpdArray.keys()):
            #
    ...
    def Write(self, FilePath):
        ...
        try:
            ...
            Pcds = list(self._VpdArray.keys())
            Pcds.sort()
            for Pcd in Pcds:
                i = 0
                for Offset in self._VpdArray[Pcd]:
                    PcdValue = str(Pcd.SkuInfoList[list(Pcd.SkuInfoList.keys())[i]].DefaultValue).strip()
                    if PcdValue == "" :
                        PcdValue  = Pcd.DefaultValue

                    fd.write("%s.%s|%s|%s|%s|%s  \n" % (Pcd.TokenSpaceGuidCName, Pcd.TokenCName, str(list(Pcd.SkuInfoList.keys())[i]),str(Offset).strip(), str(Pcd.MaxDatumSize).strip(),PcdValue))
                    i += 1
        except:
            EdkLogger.error("VpdInfoFile",
    ...
    def Read(self, FilePath):
        ...
        for Line in Lines:
            ...
            for VpdObject in list(self._VpdArray.keys()):
                for sku in list(VpdObject.SkuInfoList.keys()):
                    if VpdObject.TokenSpaceGuidCName == TokenSpaceName and VpdObject.TokenCName == PcdTokenName.strip() and sku == SkuId:
                        if self._VpdArray[VpdObject][list(VpdObject.SkuInfoList.keys()).index(sku)] == "*":
                            if Offset == "*":
                            ...
                            self._VpdArray[VpdObject][list(VpdObject.SkuInfoList.keys()).index(sku)] = Offset
                        Found = True
            if not Found:
    ...
    def GetCount(self):
        Count = 0
        for OffsetList in list(self._VpdArray.values()):
