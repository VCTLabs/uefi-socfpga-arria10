...
class DependencyRules(object):
        ...
        for Dep in ModuleObj.GetPackageDependencyList():
            ...
            if not Exist:
                if DpObj == None:
                    Result = False
                    break
                for GuidVerPair in list(DpObj.PackageSurfaceArea.keys()):
    ...
    def CheckPackageDepexSatisfied(self, PkgObj, DpObj=None):
        ...
        for ModKey in list(ModuleDict.keys()):
    ...
    def CheckDpDepexSatisfied(self, DpObj):
        for PkgKey in list(DpObj.PackageSurfaceArea.keys()):
        ...
        for ModKey in list(DpObj.ModuleSurfaceArea.keys()):
