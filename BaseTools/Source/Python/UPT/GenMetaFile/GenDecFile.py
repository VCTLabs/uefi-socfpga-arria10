...
def PackageToDec(Package, DistHeader = None):
    ...
    Statement = ('%s ' % TAB_DEC_DEFINES_DEC_SPECIFICATION).ljust(LeftOffset) + '= %s' % '0x00010017'
    SpecialItemList.append(Statement)

    BaseName = Package.GetBaseName()
    if BaseName.startswith('.') or BaseName.startswith('-'):
        BaseName = '_' + BaseName
    Statement = ('%s ' % TAB_DEC_DEFINES_PACKAGE_NAME).ljust(LeftOffset) + '= %s' % BaseName
    SpecialItemList.append(Statement)

    Statement = ('%s ' % TAB_DEC_DEFINES_PACKAGE_VERSION).ljust(LeftOffset) + '= %s' % Package.GetVersion()
    SpecialItemList.append(Statement)

    Statement = ('%s ' % TAB_DEC_DEFINES_PACKAGE_GUID).ljust(LeftOffset) + '= %s' % Package.GetGuid()
    SpecialItemList.append(Statement)

    if Package.UNIFlag:
        Statement = ('%s ' % TAB_DEC_DEFINES_PKG_UNI_FILE).ljust(LeftOffset) + '= %s' % Package.GetBaseName() + '.uni'
        SpecialItemList.append(Statement)
