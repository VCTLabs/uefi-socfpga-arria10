...
class DscBuildData(PlatformBuildClassObject):
    ...
    def _GetLoadFixAddress(self):
        ...
        #
        if 'FIX_LOAD_TOP_MEMORY_ADDRESS' in list(GlobalData.gCommandLineDefines.keys()):
            try:
    ...
    def _GetPcd(self, Type):
        ...
        PcdsKeys = list(PcdValueDict.keys())
    ...
    def _GetDynamicPcd(self, Type):
        ...
        for PcdCName, TokenSpaceGuid, SkuName, Dummy4 in PcdList:
            ...
            if (PcdCName,TokenSpaceGuid) in list(Pcds.keys()):
        ...
        for pcd in list(Pcds.values()):
            pcdDecObject = self._DecPcds[pcd.TokenCName,pcd.TokenSpaceGuidCName]
            if 'DEFAULT' not in list(pcd.SkuInfoList.keys()) and 'COMMON' not in list(pcd.SkuInfoList.keys()):
                valuefromDec = pcdDecObject.DefaultValue
                SkuInfo = SkuInfoClass('DEFAULT', '0', '', '', '', '', '', valuefromDec)
                pcd.SkuInfoList['DEFAULT'] = SkuInfo
            elif 'DEFAULT' not in list(pcd.SkuInfoList.keys()) and 'COMMON' in list(pcd.SkuInfoList.keys()):
                pcd.SkuInfoList['DEFAULT'] = pcd.SkuInfoList['COMMON']
                del(pcd.SkuInfoList['COMMON'])
            elif 'DEFAULT' in list(pcd.SkuInfoList.keys()) and 'COMMON' in list(pcd.SkuInfoList.keys()):
                del(pcd.SkuInfoList['COMMON'])
            if SkuObj.SkuUsageType == SkuObj.SINGLE:
                if 'DEFAULT' in list(pcd.SkuInfoList.keys()) and SkuObj.SystemSkuId not in list(pcd.SkuInfoList.keys()):
    ...
    def _GetDynamicHiiPcd(self, Type):
        ...
        for pcd in list(Pcds.values()):
            ...
            for sku in list(pcd.SkuInfoList.values()):
            ...
            if pcdDecObject.DatumType == 'VOID*':
                for (skuname,skuobj) in list(pcd.SkuInfoList.items()):
                    datalen = 0
...
#                                       File=self.MetaFile, Line=LineNo)
    ...
    def _GetIncludes(self):
        if self._Includes == None:
            ...
            Macros = self._Macros
            if 'PROCESSOR' in list(GlobalData.gEdkGlobal.keys()):
