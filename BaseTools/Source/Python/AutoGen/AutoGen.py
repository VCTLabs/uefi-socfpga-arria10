        ...
        for Fv in Fdf.Profile.FvDict:
            ...
            for FfsFile in Fdf.Profile.FvDict[Fv].FfsList:
                if FfsFile.InfFileName and FfsFile.NameGuid == None:
                    ...
                    for Pa in self.AutoGenObjectList:
                        ...
                        for Module in Pa.ModuleAutoGenList:
                            if path.normpath(Module.MetaFile.File) == path.normpath(FfsFile.InfFileName):
                                InfFoundFlag = True
                                if not Module.Guid.upper() in list(_GuidDict.keys()):
                    ...
                    if not InfFoundFlag:
                        if FfsFile.InfFileName.find('$') == -1:
                            ...
                            if not InfObj.Guid.upper() in list(_GuidDict.keys()):
                ...
                if FfsFile.NameGuid != None:
                            ...
                            if not PcdFoundFlag:
                                for PcdItem in Pa.AllPcdList:
                                    if (PcdItem.TokenSpaceGuidCName + "." + PcdItem.TokenCName) == _PcdName:
                                        ...
                                        if not _PcdGuidString.upper() in list(_GuidDict.keys()):
                    ...
                    if not FfsFile.NameGuid.upper() in list(_GuidDict.keys()):
...
class PlatformAutoGen(AutoGen):
    ...
    def CollectPlatformDynamicPcds(self):
        ...
        for F in list(self.Platform.Modules.keys()):
            ...
            for PcdFromModule in M.ModulePcdList+M.LibraryPcdList:
                ...
                if (PcdFromModule.TokenCName, PcdFromModule.TokenSpaceGuidCName) in list(self.Platform.Pcds.keys()):
        ...
        DscModuleList = []
        for ModuleInf in list(self.Platform.Modules.keys()):
        ...
        if (self.Workspace.ArchList[-1] == self.Arch):
            for Pcd in self._DynamicPcdList:
                ...
                Sku      = Pcd.SkuInfoList[list(Pcd.SkuInfoList.keys())[0]]
            ...
            PlatformPcds = list(self.Platform.Pcds.keys())
            PlatformPcds.sort()
            #
            ...
            for PcdKey in PlatformPcds:
                ...
                if Pcd.Type in [TAB_PCDS_DYNAMIC_VPD, TAB_PCDS_DYNAMIC_EX_VPD] and \
                   PcdKey in VpdPcdDict:
                    ...
                    for (SkuName,Sku) in list(Pcd.SkuInfoList.items()):
                        Sku.VpdOffset = Sku.VpdOffset.strip()
            ...
            for DscPcd in PlatformPcds:
                ...
                if DscPcdEntry.Type in [TAB_PCDS_DYNAMIC_VPD, TAB_PCDS_DYNAMIC_EX_VPD]:
                    if not (self.Platform.VpdToolGuid == None or self.Platform.VpdToolGuid == ''):
                        FoundFlag = False
                        for VpdPcd in list(VpdFile._VpdArray.keys()):
                        ...
                        if not FoundFlag :
                            # just pick the a value to determine whether is unicode string type
                            for (SkuName,Sku) in list(DscPcdEntry.SkuInfoList.items()):
                                Sku.VpdOffset = Sku.VpdOffset.strip()
...
#                                Sku = DscPcdEntry.SkuInfoList[DscPcdEntry.SkuInfoList.keys()[0]]
            ...
            if VpdFile.GetCount() != 0:
                ...
                if not os.path.exists(VpdFilePath) or os.path.getmtime(VpdFilePath) < DscTimeStamp:
                    ...
                    BPDGToolName = None
                    for ToolDef in list(self.ToolDefinition.values()):
    ...
    def ResolveLibraryReference(self, Module):
        ...
        while len(LibraryConsumerList) > 0:
            ...
            for LibraryName in M.Libraries:
                ...
                if Library == None:
                    for Key in list(self.Platform.LibraryClasses.data.keys()):
...
class ModuleAutoGen(AutoGen):
    ...
    def _GetDepexExpresionString(self):
        ...
        for M in [self.Module] + self.DependentLibraryList:
            ...
            for DepexExpresion in DepexExpresionList:
                for key in list(DepexExpresion.keys()):
                    Arch, ModuleType = key
    ...
    def _GetPcdsMaybeUsedByVfr(self):
        ...
        if not NameGuids:
            return []
        HiiExPcds = []
        for Pcd in list(self.PlatformInfo.Platform.Pcds.values()):
