...
class CommonUtils:
    ...
    def FileConversionFinished(self, pkg, module, src, dst):
        ...
        prefix = ' '.join([a for a in [pkg, module] if a])
        message = ''
...
class ConvertAsmFile(CommonUtils):
    ...
    def TranslateAsm(self, oldAsm, endOfLine):
        ...
        if oldAsm == '':
            ...
            if uses is not None:
                uses = [_f for _f in uses.split() if _f]
            else:
                uses = tuple()
            self.uses = uses
            ...
            publics = [p.split(':')[0].strip() for p in publics]
...
class ConvertInfFiles(CommonUtils):
    ...
    def __init__(self, infs, clone):
        CommonUtils.__init__(self, clone)
        infs = [ConvertInfFile(i, self) for i in infs]
        infs = [i for i in infs if len(i) > 0]
        dstToInfs = {'order': []}
        for inf in infs:
            for dst in inf:
